...
💡注意💡：当里面的回调函数用到了状态数据时，一定要记得将该状态变量添加进依赖项里，使得状态变更，重新以最新状态值更新回调函数，
         否则回调函数里面的状态变量永远是第一次渲染时的初始值。
...
/* 问题：test1 代码中，直接将业务逻辑以匿名函数的形式传给了Button，先不说业务逻辑与UI耦合，
这种形式还存在一个问题就是当test组件每次重新渲染的时候，都会生成一个新的重复的匿名箭头函数，
导致Button组件的 onClick 属性变更，从而触发Button组件的重新渲染 （渲染结果没有变化） */
...
/* 问题：test2代码中，将业务逻辑抽离出来了，传递引用地址给组件。但是点击button组件时，
由于更改了count state的值，会导致组件test2重新渲染，函数组件的重新渲染就是整个函数的重新执行，
因此又重新定义了新的 onChange 函数，导致Input组件的onChange属性变化了 
（因为函数是对象类型，每次新定义的函数引用地址是不一样的），从而导致Input组件做了不必要的渲染
（因为点击button,只是更改了count的值，与Input相关的props都没有改变） */
